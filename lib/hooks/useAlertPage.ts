...
async function uploadSessionPhoto(file: File, userId: number, sessionId: number, token: string): Promise<string | null> {
  try {
...
export function useAlertPage(user: any, token: string | null) {
  const now = new Date();
  ...
  const [logForm, setLogForm] = useState<LogForm>({ rating: 0, wind_feel: "", gear_type: "", gear_size: "", duration_minutes: 0, notes: "" });
      ...
      for (const alert of alertsNeedingHourly) {
        ...
        for (const spot of spots) {
          ...
          try {
            let omRes;
            if (isPastDate) {
              omRes = await fetch(`https://archive-api.open-meteo.com/v1/archive?latitude=${latitude}&longitude=${longitude}&start_date=${alert.target_date}&end_date=${alert.target_date}&hourly=wind_speed_10m,wind_gusts_10m,wind_direction_10m&wind_speed_unit=kn&timezone=Europe/Amsterdam`);
            } else {
              omRes = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=wind_speed_10m,wind_gusts_10m,wind_direction_10m&wind_speed_unit=kn&timezone=Europe/Amsterdam&forecast_days=7`);
            }
          ...
          if (spot_type?.toLowerCase() === "zee" && !tideMap[key]) {
            if (isPastDate) {
              ...
              if (storedTides?.length) {
                tideMap[key] = { extremes: storedTides.map((t: any) => ({ time: t.time, type: t.type, height: t.height })) };
              }
            } else {
              try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/sessions?created_by=eq.${user.id}&select=id,spot_id,session_date,status,rating,photo_url`, {
          headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${token}` },
        });
  ...
  const handleIkGa = async (spotId: number, date: string, alertId: number, wind: number, gust: number, dir: string) => {
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/sessions`, {
        ...
        body: JSON.stringify({ created_by: user.id, spot_id: spotId, session_date: date, alert_id: alertId, status: "going", going_at: new Date().toISOString(), forecast_wind: wind, forecast_gust: gust, forecast_dir: dir }),
      });
      if (res.ok) {
        const data = await res.json();
