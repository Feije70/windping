  ...
  return (
    <div style={{ display: "flex", gap: 4, padding: "0 40px", marginBottom: 28 }}>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i < current ? C.sky : C.cardBorder, transition: "background 0.3s ease" }} />
      ))}
    </div>
  );
}
...
function SessionLogInner() {
  ...
  async function fetchSpotName(spotId: number): Promise<string> {
    if (!token) return "";
    try {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      ...
      if (sessionIdParam) {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/sessions?id=eq.${sessionIdParam}&created_by=eq.${uid}&select=id,spot_id,session_date,status,rating,gear_type,gear_size,notes,forecast_wind,forecast_dir,wind_feel,image_url`, {
          headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        if (data?.length) {
          const name = await fetchSpotName(data[0].spot_id);
  ...
  const saveSession = async () => {
    if (!session || !rating || !token) return;
    setSaving(true);
    setError("");
    try {
  ...
  const skipSession = async () => {
    ...
    await fetch("/api/sessions", {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ session_id: session.id, status: "skipped" }),
    });
    const nextIndex = currentIndex + 1;
    if (nextIndex < sessions.length) {
      setCurrentIndex(nextIndex);
  ...
  if (step === 7) {
    ...
    const handleShare = async () => {
      if (!canShare) { await navigator.clipboard.writeText(shareUrl); alert("Link gekopieerd!"); return; }
      try { await navigator.share({ title: `WindPing — ${saved?.spotName || "Sessie"}`, text: shareText, url: shareUrl }); } catch {}
    };
    ...
    return (
      <div style={{ background: C.cream, minHeight: "100vh", padding: "40px 20px 60px" }}>
        <div style={{ maxWidth: 380, margin: "0 auto" }}>
          ...
          <div style={{ background: C.navy, borderRadius: 20, overflow: "hidden", marginBottom: 20, boxShadow: "0 8px 32px rgba(31,53,76,0.25)" }}>
            {saved?.imageUrl && <img src={saved.imageUrl} alt="" style={{ width: "100%", height: 200, objectFit: "cover", display: "block" }} />}
            <div style={{ padding: "16px 18px 18px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                ...
                {saved?.rating && <div style={{ fontSize: 16, fontWeight: 800, color: C.green, marginTop: 16 }}>{ratingLabels[saved.rating]}</div>}
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
            <button onClick={handleShare} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "15px", background: C.sky, color: "#fff", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 16px ${C.sky}40` }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>
              {canShare ? "Deel sessie" : "Kopieer link"}
            </button>
            <button onClick={handleWhatsApp} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "15px", background: "#25D366", color: "#fff", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
    ...
    <div style={{ background: C.cream, minHeight: "100vh", color: C.navy }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "20px 16px 80px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          ...
          {sessions.length > 1 && <span style={{ fontSize: 11, color: C.muted, fontWeight: 600 }}>{currentIndex + 1} / {sessions.length}</span>}
          <button onClick={skipSession} style={{ fontSize: 12, color: C.muted, background: "none", border: "none", cursor: "pointer", fontWeight: 500 }}>Overslaan</button>
        </div>
        ...
        {step === 3 && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            <p style={{ textAlign: "center", fontSize: 13, color: C.sub, margin: "0 0 24px" }}>Wat had je onder je voeten?</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <button onClick={() => { setBoardOrFoil("foil"); setStep(5); }} style={{ padding: "28px 16px", borderRadius: 16, border: `2px solid ${boardOrFoil === "foil" ? C.sky : C.cardBorder}`, background: boardOrFoil === "foil" ? `${C.sky}12` : C.card, cursor: "pointer", transition: "all 0.2s", boxShadow: boardOrFoil === "foil" ? `0 4px 16px ${C.sky}25` : C.cardShadow, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              ...
              <button onClick={() => { setBoardOrFoil("board"); setStep(4); }} style={{ padding: "28px 16px", borderRadius: 16, border: `2px solid ${boardOrFoil === "board" ? C.sky : C.cardBorder}`, background: boardOrFoil === "board" ? `${C.sky}12` : C.card, cursor: "pointer", transition: "all 0.2s", boxShadow: boardOrFoil === "board" ? `0 4px 16px ${C.sky}25` : C.cardShadow, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                <svg width="52" height="52" viewBox="0 0 52 52" fill="none">
                  <path d="M26 8 Q38 10 40 26 Q38 42 26 44 Q14 42 12 26 Q14 10 26 8Z" fill={boardOrFoil === "board" ? `${C.sky}15` : "#B0BAC515"} stroke={boardOrFoil === "board" ? C.sky : "#B0BAC5"} strokeWidth="1.8"/>
          ...
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {boardTypesForPropulsion.map(bt => (
                <button key={bt.id} onClick={() => { setBoardType(bt.id); setStep(5); }} style={{ padding: "16px 20px", borderRadius: 14, textAlign: "left", border: `2px solid ${boardType === bt.id ? C.sky : C.cardBorder}`, background: boardType === bt.id ? `${C.sky}10` : C.card, cursor: "pointer", transition: "all 0.2s", boxShadow: boardType === bt.id ? `0 4px 16px ${C.sky}20` : C.cardShadow }}>
          ...
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ position: "relative" }}>
                <input type="text" inputMode="decimal" value={sailSize} onChange={e => setSailSize(e.target.value.replace(/[^0-9.,]/g, "").replace(",", "."))} placeholder={propulsion === "zeil" ? "7.6" : propulsion === "wing" ? "5.0" : "12"}
                  style={{ width: "100%", padding: "13px 48px 13px 16px", borderRadius: 12, border: `1.5px solid ${sailSize ? C.sky : C.cardBorder}`, fontSize: 16, color: C.navy, background: C.card, boxSizing: "border-box", outline: "none" }} />
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ position: "relative" }}>
                <input type="text" inputMode="numeric" value={boardLength} onChange={e => setBoardLength(e.target.value.replace(/[^0-9]/g, ""))} placeholder={propulsion === "zeil" ? "245" : "138"}
                  style={{ width: "100%", padding: "13px 48px 13px 16px", borderRadius: 12, border: `1.5px solid ${boardLength ? C.sky : C.cardBorder}`, fontSize: 16, color: C.navy, background: C.card, boxSizing: "border-box", outline: "none" }} />
          ...
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {WIND_FEELS.map(w => (
                  <button key={w.id} onClick={() => setWindFeel(windFeel === w.id ? null : w.id)} style={{ flex: 1, padding: "12px 6px", borderRadius: 12, border: `2px solid ${windFeel === w.id ? C.sky : C.cardBorder}`, background: windFeel === w.id ? `${C.sky}12` : C.card, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Hoe was het water? Iets bijzonders? Tips voor de volgende keer?" rows={3}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {photoUrl ? (
                ...
                <div onClick={() => { if (!photoUploading) document.getElementById("photo-upload-input")?.click(); }}
                  ...
                  {photoUploading ? (
                    ...
                    <>
                      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.muted} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
            ...
            <div style={{ padding: "14px 16px", background: C.card, borderRadius: 14, boxShadow: C.cardShadow, marginBottom: 20, border: `1px solid ${C.cardBorder}` }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                {rating && <div style={{ fontSize: 12, fontWeight: 800, color: RATING_COLORS[rating], minWidth: 44, textAlign: "right" }}>{RATINGS.find(r => r.value === rating)?.label}</div>}
              </div>
            </div>

            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={goBack} style={{ padding: "14px", background: C.card, color: C.sub, border: `1px solid ${C.cardBorder}`, borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: "pointer", width: 60 }}>←</button>
              <button onClick={saveSession} disabled={saving} style={{ flex: 1, padding: "14px", background: C.green, color: "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
