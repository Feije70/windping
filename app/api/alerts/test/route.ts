...
async function fetchHourlyForDay(lat: number, lng: number, dateStr: string): Promise<HourlyEntry[]> {
  try {
    ...
    if (!res.ok) return [];
    const data = await res.json() as { hourly: { time: string[]; wind_speed_10m: number[]; wind_gusts_10m: number[]; wind_direction_10m: number[] } };
    ...
    }).filter((h): h is HourlyEntry => h !== null);
  } catch { return []; }
}
...
async function fetchDailyMax(lat: number, lng: number, dateStr: string) {
  try {
    ...
    const data = await res.json() as { daily: { time: string[]; wind_speed_10m_max: number[]; wind_gusts_10m_max: number[]; wind_direction_10m_dominant: number[] } };
...
async function fetchTides(lat: number, lng: number, dateStr: string): Promise<TideEntry[]> {
  ...
  try {
    ...
    if (!res.ok) return [];
    const data = await res.json() as { data: { time: string; type: string }[] };
    return (data.data || [])
      .filter(e => new Date(e.time).toLocaleDateString("en-CA", { timeZone: "Europe/Amsterdam" }) === dateStr)
      .map(e => ({ time: new Date(e.time).toLocaleTimeString("nl-NL", { timeZone: "Europe/Amsterdam", hour: "2-digit", minute: "2-digit" }), type: e.type === "high" ? "HW" : "LW" }));
  } catch { return []; }
}
...
function buildEmailHtml(
  ...
  spotData: SpotData[], hourlyBySpot: Record<string, HourlyEntry[]>, tideBySpot: Record<string, TideEntry[]>,
  ...
  const hourlyTables = spotData.map(s => {
    ...
    const wMin = s.userWindMin || 12;
    const hourCells = hours.map(h => `<td style="padding:4px 6px;text-align:center;border-bottom:1px solid #E8E0D8;color:#6B7B8F;font-size:11px;">${h.hour}:00</td>`).join("");
    const windCells = hours.map(h => `<td style="padding:4px 6px;text-align:center;border-bottom:1px solid #E8E0D8;color:${h.wind >= wMin ? "#3EAA8C" : "#8A9BB0"};font-weight:600;font-size:12px;">${h.wind}</td>`).join("");
    const gustCells = hours.map(h => `<td style="padding:4px 6px;text-align:center;border-bottom:1px solid #E8E0D8;color:#6B7B8F;font-size:11px;">${h.gust}</td>`).join("");
    const dirCells = hours.map(h => `<td style="padding:4px 6px;text-align:center;color:#8A9BB0;font-size:10px;">${h.dir}</td>`).join("");
    const tides = tideBySpot[`${s.spotId}_${targetDate}`] || [];
    const tideHtml = tides.length > 0 ? `<div style="margin-top:4px;font-size:11px;color:#2E8FAE;">🌊 ${tides.map(t => `${t.type} ${t.time}`).join(" · ")}</div>` : "";
...
export async function POST(req: Request) {
  ...
  const body = await req.json() as { action: string; userId: number; alertType?: string; spotId?: number };
  ...
  if (action === "send_test") {
    ...
    const allSpotIds = (userSpots as { spot_id: number }[] || []).map(x => x.spot_id);
    
    let spotsToUse: SpotRow[] = [];
    if (spotId) {
      const { data } = await sb.from("spots").select("id, display_name, latitude, longitude").eq("id", spotId).single();
      if (data) spotsToUse = [data as SpotRow];
    } else if (allSpotIds.length) {
      const { data } = await sb.from("spots").select("id, display_name, latitude, longitude").in("id", allSpotIds);
      spotsToUse = (data as SpotRow[]) || [];
    }
    if (!spotsToUse.length) spotsToUse = [{ id: 0, display_name: "Test Spot", latitude: 52.46, longitude: 4.56 }];
    
    const { data: idealConds } = await sb.from("ideal_conditions").select("*").eq("user_id", userId).in("spot_id", spotsToUse.map(s => s.id));
    const condsMap: Record<number, IdealCondition> = {};
    (idealConds as IdealCondition[] || []).forEach(c => { condsMap[c.spot_id] = c; });
    ...
    const hourlyBySpot: Record<string, HourlyEntry[]> = {};
    const tideBySpot: Record<string, TideEntry[]> = {};
    ...
    for (const s of spotsToUse) {
      ...
      const rawDirs: string[] | boolean[] = ic?.directions?.length ? ic.directions : [];
      let dirOk = true;
      if (rawDirs.length > 0) {
        if (typeof rawDirs[0] === "string") {
          dirOk = (rawDirs as string[]).includes(daily.dir);
        } else {
    ...
    const { data: alert, error } = await sb.from("alert_history").insert({
      ...
      spot_ids: spotData.map(s => s.spotId),
    ...
    if (RESEND_API_KEY && user.email) {
      ...
      } catch (e) { emailResult = `error: ${getErrorMessage(e)}`; }
    }
  ...
  if (action === "test_push") {
    ...
    for (const s of (spots as SpotRow[] || [])) {
    ...
    try {
      ...
      for (const sub of subs) {
        try {
          ...
          results.push({ endpoint: sub.endpoint.slice(0, 60) + "...", success: false, statusCode: err.statusCode, error: err.body || getErrorMessage(pe) });
          if (err.statusCode === 404 || err.statusCode === 410) {
            await sb.from("push_subscriptions").delete().eq("endpoint", sub.endpoint);
          }
        }
      }
    } catch (e) { return NextResponse.json({ error: "web-push failed", detail: getErrorMessage(e) }); }
