...
export async function GET(req: NextRequest) {
  ...
  try {
    // 1. Haal spots op met nieuws score >= 7, niet geblokkeerd
    ...
    const spotsRes = await sbFetch(`spots?id=in.(${spotIds.join(",")})&select=id,display_name,country`);
    const spotsData = await spotsRes.json();
    const spotMap: Record<number, any> = {};
    if (Array.isArray(spotsData)) spotsData.forEach((s: any) => { spotMap[s.id] = s; });

    // 3. Haal gebruikers op voor deze spots
    const userSpotsRes = await sbFetch(`user_spots?spot_id=in.(${spotIds.join(",")})&select=user_id,spot_id`);
    ...
    return NextResponse.json({ pushed: totalPushed, spots_with_news: spotsWithNews.length, push_log: pushLog });
