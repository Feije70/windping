...
const matchBadgeBg = { epic: C.epicBg, go: C.goBg, maybe: C.epicBg, no: C.terraTint };
...
function SpotCard({ d, expanded, onToggle }: { d: SpotData; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <div style={{ background: C.card, borderRadius: 16, boxShadow: C.cardShadow, overflow: "hidden", transition: "all 0.25s", position: "relative" }}>
      <div onClick={onToggle} style={{ display: "flex", alignItems: "center", padding: "16px 16px", gap: 12, cursor: "pointer", position: "relative" }}>
        <div style={{ width: 52, height: 52, borderRadius: 13, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0, background: matchBadgeBg[match], border: `1.5px solid ${mColor}30`, ...(match === "epic" ? { boxShadow: `0 0 12px ${mColor}25` } : {}) }}>
        ...
        <div style={{ textAlign: "center", padding: "5px 10px", borderRadius: 8, background: matchBadgeBg[match], flexShrink: 0 }}>
            ...
            {Array.from({ length: endIdx - nowIdx }, (_, i) => {
              ...
              return (
                <div key={idx} style={{ flex: "0 0 auto", textAlign: "center", padding: "6px 5px", minWidth: 40 }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 800, color: fColor, background: `${fColor}15`, borderRadius: 6, padding: "2px 5px", marginBottom: 3 }}>{fW}</div>
            ...
            <div style={{ marginTop: 14, padding: "12px 14px", background: C.oceanTint, borderRadius: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4 }}>
                {d.tide.extremes.slice(0, 8).map((ex, i) => {
                  ...
                  return (
                    <div key={i} style={{ flex: "0 0 auto", padding: "7px 10px", borderRadius: 10, minWidth: 60, textAlign: "center", background: isHW ? "rgba(46,111,126,0.08)" : C.card, border: `1px solid ${isHW ? "rgba(46,111,126,0.12)" : C.cardBorder}`, opacity: isPast ? 0.5 : 1 }}>
                      ...
                      <div style={{ fontSize: 10, fontWeight: 700, color: isHW ? C.sky : C.muted, marginTop: 1 }}>{isHW ? "▲ HW" : "▼ LW"}</div>
                      {ex.height != null && <div style={{ fontSize: 9, color: C.muted, marginTop: 1 }}>{(ex.height as number).toFixed(1)}m</div>}
                    </div>
                  );
                })}
              </div>
...
export default function CheckPage() {
  const { user, authId, token, loading: authLoading } = useUser();

  const [status, setStatus] = useState<"loading" | "empty" | "error" | "done">("loading");
  ...
  const loadWeather = useCallback(async () => {
    ...
    try {
      const uid = user.id;
      const userMinTemp = (user as any).min_temperature ?? null;
      ...
      async function fetchTide(s: Spot): Promise<SpotData["tide"] | undefined> {
        ...
        try {
          const [slRes, exRes] = await Promise.all([
            fetch(`/api/tide?spot_id=${s.id}&lat=${s.latitude}&lng=${s.longitude}&type=sea_level`, { headers: { Authorization: `Bearer ${token}` } }),
            fetch(`/api/tide?spot_id=${s.id}&lat=${s.latitude}&lng=${s.longitude}&type=extremes`, { headers: { Authorization: `Bearer ${token}` } }),
          ]);
    ...
    <div style={{ background: C.cream, minHeight: "100vh", color: C.navy }}>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "24px 16px 100px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <a href="/" style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: 10, background: C.card, boxShadow: "0 1px 3px rgba(31,53,76,0.08)", border: `1px solid ${C.cardBorder}`, textDecoration: "none", flexShrink: 0 }}>
        ...
        {status === "done" && spots.length > 2 && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: C.card, borderRadius: 12, padding: "0 14px", boxShadow: "0 1px 3px rgba(31,53,76,0.06)", border: `1.5px solid ${C.cardBorder}`, marginBottom: 12, height: 44 }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.muted} strokeWidth="2.5" strokeLinecap="round" style={{ flexShrink: 0 }}><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
            <input type="text" placeholder="Zoek een spot…" value={search} onChange={e => setSearch(e.target.value)}
              style={{ flex: 1, border: "none", background: "transparent", outline: "none", fontSize: 14, fontWeight: 500, color: C.navy, fontFamily: fonts.body }} />
            {search && <button onClick={() => setSearch("")} style={{ border: "none", background: "none", cursor: "pointer", padding: 0, color: C.muted, fontSize: 16, lineHeight: 1 }}>×</button>}
          </div>
        )}
        ...
        {status === "done" && spots.length > 1 && (
          <div style={{ display: "flex", background: C.creamDark, borderRadius: 10, padding: 3, gap: 2, marginBottom: 16 }}>
            {(["status", "mine"] as const).map((mode) => (
              <button key={mode} onClick={() => setSortMode(mode)} style={{ flex: 1, padding: "8px 12px", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", background: sortMode === mode ? C.card : "transparent", color: sortMode === mode ? C.navy : C.muted, boxShadow: sortMode === mode ? "0 1px 3px rgba(31,53,76,0.06)" : "none", transition: "all 0.15s" }}>
        ...
        {status === "empty" && (
          <div style={{ textAlign: "center", padding: "40px 20px", background: C.card, borderRadius: 16, boxShadow: C.cardShadow, border: `1.5px dashed ${C.cardBorder}` }}>
            ...
            <a href="/spots" style={{ display: "inline-block", padding: "10px 24px", background: C.sky, color: "#fff", fontSize: 13, fontWeight: 600, borderRadius: 10, textDecoration: "none" }}>Ontdek spots →</a>
          </div>
        )}
        ...
        {status === "error" && (
          <div style={{ textAlign: "center", padding: "40px 20px", background: C.card, borderRadius: 16, boxShadow: C.cardShadow }}>
            ...
            <button onClick={loadWeather} style={{ padding: "8px 20px", background: C.sky, color: "#fff", fontSize: 13, fontWeight: 600, borderRadius: 8, border: "none", cursor: "pointer" }}>Probeer opnieuw</button>
          </div>
        )}
        ...
        {status === "done" && (
          <>
            ...
            <button onClick={loadWeather} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, width: "100%", marginTop: 14, padding: "12px 20px", background: C.card, boxShadow: "0 1px 3px rgba(31,53,76,0.04)", borderRadius: 12, fontSize: 13, fontWeight: 600, color: C.sky, cursor: "pointer", border: "none" }}>
