...
function SessionCard({ s, onClick }: { s: Session; onClick: () => void }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div onClick={onClick} style={{ background: C.card, boxShadow: C.cardShadow, borderRadius: 16, overflow: "hidden", cursor: "pointer", borderLeft: s.rating ? `3px solid ${ratingColors[s.rating]}` : `3px solid ${C.cardBorder}` }}>
        ...
        <div style={{ padding: "12px 14px" }}>
          ...
          {s.notes && (
            <div style={{ fontSize: 12, color: C.sub, marginTop: 8, lineHeight: 1.5, borderTop: `1px solid ${C.cardBorder}`, paddingTop: 8, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
              {s.notes}
            </div>
          )}
        </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
        ...
        <a href={waUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "9px", background: C.card, border: `1.5px solid ${C.cardBorder}`, borderRadius: 10, color: C.sub, fontSize: 12, fontWeight: 700, textDecoration: "none" }}>
...
function SessionDetail({ s, token, onClose }: { s: Session; token: string; onClose: () => void }) {
  const spot = s._spotName || "Spot";
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, background: C.cream, overflowY: "auto" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: C.cream, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
        <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: "50%", background: C.card, border: `1px solid ${C.cardBorder}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
      ...
      <div style={{ padding: "0 16px 120px" }}>
        ...
        <div style={{ background: C.card, boxShadow: C.cardShadow, borderRadius: 16, padding: "16px", marginBottom: 12 }}>
          ...
          {s.rating && <div style={{ fontSize: 18, fontWeight: 800, color: ratingColors[s.rating] }}>{ratingLabels[s.rating]}</div>}
        </div>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 20, marginBottom: 12 }}>
          <button onClick={handleShare} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "13px", background: "#2E8FAE", border: "none", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
          ...
          <a href={waUrl} target="_blank" rel="noopener noreferrer" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "13px", background: C.card, border: `1.5px solid ${C.cardBorder}`, borderRadius: 12, color: C.sub, fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
...
export default function MijnSessiesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`${SUPABASE_URL}/rest/v1/sessions?created_by=eq.${uid}&status=eq.completed&order=id.desc&select=id,spot_id,session_date,status,rating,gear_type,gear_size,forecast_wind,forecast_dir,wind_feel,notes,photo_url,photo_crop,image_url`, {
  ...
  return (
    <div style={{ background: C.cream, minHeight: "100vh" }}>
      ...
      {selected && <SessionDetail s={selected} token={token!} onClose={() => setSelected(null)} />}
      <NavBar />
