  ...
  const loadData = useCallback(async () => {
    try {
      ...
      try {
        const token = await getValidToken();
        if (token) {
          const sessRes = await fetch(`${SUPABASE_URL}/rest/v1/sessions?created_by=eq.${user.id}&order=id.desc&limit=10&select=id,spot_id,session_date,status,rating,gear_type,forecast_wind,forecast_dir,photo_url`, {
